...
module.exports = function (Groups) {
	Groups.getMembers = async function (groupName, start, stop) {
		const gMembers = await db.getSortedSetRevRange(
	...
	Groups.getMemberUsers = async function (groupNames, start, stop) {
		async function get(groupName) {
			...
			const demo = await user.getUsersFields(uids, [
				"uid",
				"username",
				"picture",
				"userslug",
			...
			// winston.info(JSON.stringify(demo));
			var demo2 = demo.slice(0);
			demo2.sort(function (a, b) {
				return a.assetsCount - b.assetsCount;
			});
			// winston.info("+++" + JSON.stringify(demo2));
			return demo2;
		}
