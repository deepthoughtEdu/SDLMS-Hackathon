...
module.exports = function (Posts) {
	Posts.getPostSummaryByPids = async function (pids, uid, options) {
		...
		const fields = ['pid', 'tid', 'content', 'uid', 'timestamp', 'deleted', 'upvotes', 'downvotes', 'replies', 'typeOfAsset', 'questions'].concat(options.extraFields);
		...
		posts.forEach(function (post) {
			...
			//winston.info(JSON.stringify(post));
			post.user = uidToUser[post.uid];
			post.topic = tidToTopic[post.tid];
			post.category = post.topic && cidToCategory[post.topic.cid];
			if (post.typeOfAsset == "quiz") { post.isMainPost = true; post.questions = post.questions[0].question.title; }
			else { post.isMainPost = post.topic && post.pid === post.topic.mainPid; }
