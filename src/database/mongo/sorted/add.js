...
module.exports = function (module) {
	...
	module.sortedSetsAdd = async function (keys, scores, value, memberType) {
		...
		for (var i = 0; i < keys.length; i += 1) {
			bulk.find({ _key: keys[i], value: value, "role": memberType }).upsert().updateOne({ $set: { score: parseFloat(isArrayOfScores ? scores[i] : scores) } });
		}
		await bulk.execute();
	};
