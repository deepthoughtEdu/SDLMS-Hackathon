...
module.exports = function (privileges) {
	...
	privileges.topics.get = async function (tid, uid) {
		...
		const [userPrivileges, isAdministrator, isModerator, disabled, roles] =
			await Promise.all([
		...
		return await plugins.hooks.fire("filter:privileges.topics.get", {
			"topics:reply":
				(privData["topics:reply"] &&
					((!topicData.locked && !topicData.deleted) || isModerator)) ||
				isAdministrator,
	...
	privileges.topics.canDelete = async function (tid, uid) {
		...
		const preventTopicDeleteAfterReplies =
			meta.config.preventTopicDeleteAfterReplies;
		if (
			!isModerator &&
