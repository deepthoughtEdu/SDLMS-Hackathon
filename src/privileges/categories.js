...
module.exports = function (privileges) {
	...
	privileges.categories.list = async function (cid) {
		async function getLabels() {
			return await utils.promiseParallel({
				users: plugins.hooks.fire(
					...
					privileges.privilegeLabels.slice()
				),
		...
		const keys = await utils.promiseParallel({
			users: plugins.hooks.fire(
				...
				privileges.userPrivilegeList.slice()
			),
			groups: plugins.hooks.fire(
		...
		payload.columnCountUserOther =
			payload.labels.users.length - privileges.privilegeLabels.length;
		payload.columnCountGroup = payload.labels.groups.length + 3;
		payload.columnCountGroupOther =
			payload.labels.groups.length - privileges.privilegeLabels.length;
		return payload;
	};
	...
	privileges.categories.get = async function (cid, uid) {
		...
		const [userPrivileges, isAdministrator, isModerator, roles] =
			await Promise.all([
				helpers.isAllowedTo(privs, uid, cid),
				user.isAdministrator(uid),
				user.isModerator(uid, cid),
				user.getUsersFields([uid], []),
			]);
		...
		return await plugins.hooks.fire("filter:privileges.categories.get", {
			...privData,
			session: Array.isArray(roles) && roles.length  && roles[0].roles ?
				     (roles[0].roles.find(e => e.id == 4) ? true : false) : false,
			cid: cid,
			uid: uid,
	...
	privileges.categories.canMoveAllTopics = async function (
		currentCid,
		targetCid,
		uid
	) {
	...
	privileges.categories.userPrivileges = async function (cid, uid) {
		...
		privileges.userPrivilegeList.forEach(function (privilege) {
			tasks[privilege] = groups.isMember(
				uid,
