...
groupsController.list = async function (req, res) {
	...
	//const [groupData, allowGroupCreation, isUserAdmin] = await Promise.all([
	const [groupData, isUserAdmin] = await Promise.all([
	...
	var filterdGroups;
	if (req.uid && !isUserAdmin) {
		filterdGroups = groupData.filter(elem => elem.members.length && elem.members.findIndex(e => e.uid == req.uid) > -1)
	}
	else {
		filterdGroups = groupData;
	}
