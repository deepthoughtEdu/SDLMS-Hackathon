...
define('forum/topic/postTools', [
		...
		$('[component="topic"]').on('show.bs.dropdown', '.moderator-tools', function () {
			...
			socket.emit('posts.loadPostTools', { pid: pid, cid: ajaxify.data.cid }, function (err, data) {
				...
				app.parseAndTranslate('partials/topic/post-menu-list', data, function (html) {
					...
					if(!ajaxify.data.privileges.session && ajaxify.data.mode && ajaxify.data.mode.active == 'false'){
						filterMenu(dropdownMenu,['[component="post/edit"]'])
					}
	...
	function filterMenu(dropdown, items= ['view-history']){
		items.forEach((e)=>{
			dropdown.find(e).remove()
		})
	}
	...
	function addPostHandlers(tid) {
		...
		// if (ajaxify.data.posts[0].typeOfAsset == "quiz") { renderQuiz(); }

		// var contents = ajaxify.data.posts[0].content;
		// var form = $('[component="post/content/quiz"]');
		// try {
		// 	$.when(
		// 		$.getScript("https://ajax.googleapis.com/ajax/libs/jqueryui/1.10.3/jquery-ui.min.js"),
		// 		$.getScript("https://formbuilder.online/assets/js/form-render.min.js"),
		// 		$.Deferred(function (deferred) { $(deferred.resolve) })
		// 	).done(function () {
		// 		console.log("Imported!");
		// 			contents = contents.replace('</p>', '').replace('<p dir="auto">', '');
		// 			const formData = JSON.parse(contents);
		...
		postContainer.on('click', '[component="post/reply"]', function () {
			onReplyCustomClicked($(this), tid, 'evalaute');
		});

		postContainer.on('click', '[component="post/evalaute"]', function () {
			onReplyClicked($(this), tid);
		});

		postContainer.on('click', '[component="post/reply/spreadsheet"]', function () {
			onReplyCustomClicked($(this), tid, 'spreadsheet');
		});

		postContainer.on('click', '[component="post/reply/quiz:reply"]', function () {
			onReplyCustomClicked($(this), tid, 'quiz:reply');
		});
		...
		$('.topic').on('click', '[component="topic/reply/quiz"]', function (e) {
			e.preventDefault();
			onReplyCustomClicked($(this), tid, 'quiz');
		});

		$('.topic').on('click', '[component="topic/reply-as-spreadsheet"]', function (e) {
			e.preventDefault();
			let template = $(this).data('template');
			app.log(e,1)
			onReplyCustomClicked($(this), tid, 'spreadsheet', template);
		});
		$('.topic').on('click', '[component="topic/reply-as-threadbuilder"]', function (e) {
			e.preventDefault();
			let template = $(this).data('template');
			app.log(e,1)
			onReplyCustomClicked($(this), tid, 'threadbuilder', template);
		});

		// $('.topic').on('click', '[component="topic/as-spreadsheet"]', function () {
		// 	let template = $(this).data('template');
		// 	translator.translate('[[topic:link_back_sheet, ' + ajaxify.data.titleRaw + ', ' + config.relative_path + '/topic/' + ajaxify.data.slug + ']]', function (body) {
		// 		$(window).trigger('action:composer.sheet.new', {
		...
		// $('.topic').on('click', '[component="topic/reply-as-quiz"]', function () {
		// 	translator.translate('[[topic:link_back_quiz, ' + ajaxify.data.titleRaw + ', ' + config.relative_path + '/topic/' + ajaxify.data.slug + ']]', function (body) {
	...
	// Custom function for answering the quiz
	function onReplyCustomClicked(button, tid, type, template = 0) {
		var selectedNode = getSelectedNode();
		...
		showStaleWarning(function () {
			...
			var toPid = button.is(`[component="post/reply/${type}"]`) ? getData(button, 'data-pid') : null;
			...
			if (selectedNode.text && (!toPid || !selectedNode.pid || toPid === selectedNode.pid)) {
				...
				$(window).trigger(`action:composer.post.${type}`, {
					tid: tid,
					pid: toPid,
					topicName: ajaxify.data.titleRaw,
					text: username ? username + ' ' : ($('[component="topic/quickreply/text"]').val() || ''),
