var Comments = {
	...
	getDefaultOptions: function () {
		return {
			...
			// Colors
			highlightColor: 'var(--primary-background-color)',
	...
	createCommentingFieldElement: function (parentId, existingCommentId, isMain) {
		...
		var saveButton = $('<span/>', {
			'class': saveButtonClass + ' sdlms-button button-primary save highlight-background',
			'text': saveButtonText
		});
		...
		if (this.options.enablePinging) {
			textarea.textcomplete([{
				...
				template: function (user) {
					...
					var profilePictureEl = self.createProfilePictureElement(user.picture);
					...
					var nameEl = $('<div/>', {
						'class': 'name',
					}).html(user.fullname || user.displayName || user.username);
				...
				replace: function (user) {
					console.log(user)
					var tag = self.createTagElement('@' + (user.fullname || user.displayName || user.username), 'ping', user.id, {
						'data-user-id': user.id
					});
					return ' ' + tag[0].outerHTML + ' ';
				},
			}], {
				appendTo: `#${$(self.options.parentElement).attr('id')}`,
			...
			textarea.textcomplete([{
				...
				search: function (term, callback) {
					...
					if (!$(self.options.parentElement).length) return [];
					let threads = [];
					let nodeName = $(self.options.parentElement)[0].nodeName;
					console.log(nodeName)
					switch (nodeName) {
					case 'SDLMS-EAGLE-BUILDER':
						$(self.options.parentElement)
							.find('[thread]').find('[thread-name="title"]').each(function () {
								if ($.trim($(this).text())) {
									threads.push({
										id: 'thread-' + self.options.parentElement,
										content: $.trim($(this).text()).replaceAll('\n', '').replaceAll('\r', '').replaceAll('\t', '')
									});
								}
							});
						console.log(threads)
						break;
					case 'SDLMS-THREAD-BUILDER':
						$(self.options.parentElement)
							.find('[thread]').find('[thread-name="title"]').each(function () {
								if ($.trim($(this).text())) {
									threads.push({
					...
					case 'SDLMS-QUIZ':
						$(self.options.parentElement)
							.find('[quiztarget]').each(function () {
								var content = $(this).find('[question]')
									.clone()
									.children()
									.remove()
									.end()
					...
					callback(threads.filter(function (thread) {
						return thread.content.toLowerCase().indexOf(term.toLowerCase()) != -1;
					}));
				},
				template: function (thread) {
					var wrapper = $('<div/>');
				...
				replace: function (thread) {
					var tag = self.createTagElement('#' + thread.content.replaceAll(' ', '-_-'), 'hashtag', thread.content.replaceAll(' ', '-_-'));
					return ' ' + tag[0].outerHTML + ' ';
				},
			}], {
				appendTo: `#${$(self.options.parentElement).attr('id')}`,
			...
			$.fn.textcomplete.Dropdown.prototype.render = function (zippedData) {
				...
				function getSelectionCoords(win) {
					...
					return {
						x: x,
						y: y +16  // 16 is the line hight of the textarea
					};
	...
	createCommentJSON: function (commentingField) {
		...
		var commentJSON = {
			id: 'c' + $(this.$el).attr('id') + (this.getComments().length + 1), // Temporary id
			parent: textarea.attr('data-parent') || null,
			created: time,
			modified: time,
			content: this.getTextareaContent(textarea),
