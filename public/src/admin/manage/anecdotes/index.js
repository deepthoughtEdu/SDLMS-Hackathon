...
define("admin/manage/anecdotes/index", function () {
	//* here we are defining index module !
	var index = {};
	...
	index.init = function () {
			...
			listing: function (data) {
				return `<tr uid=${data.author_uid} tid=${data.tid} imgsrc=${
					data.author.picture
							...
							<td class="author-name text-truncate">${data.author.fullname}</td>
							<td class="anecdote-title text-truncate">${data.title}</td>
							<td class="anecdote-content text-truncate">${data.content}</td>
							<td class="anecdote-placements text-truncate">${data.placements.toString()}</td>
						</tr>`;
			},
		};
		...
		$("body").on("click", "#preview-btn", function () {
			...
			$("body").on("click", "#submit-car", function () {
		...
		$("#saved-redirect").on("click", () =>
			ajaxify.go("/admin/manage/anecdotes/saved")
		);

		$("#add-redirect").on("click", () =>
			ajaxify.go("/admin/manage/anecdotes/add")
		);
	};
