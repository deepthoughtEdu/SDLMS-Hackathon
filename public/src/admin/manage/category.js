...
define("admin/manage/category", [
	...
	Category.init = function (custom = false) {
		...
		Category.catPurge = () => {
			Benchpress.render("admin/partials/categories/purge", {
				...
				topic_count: ajaxify.data.topic_count,
			}).then((html) => {
				var modal = bootbox.dialog({
					title: "[[admin/manage/categories:purge]]",
					message: html,
					...
					buttons: {
						save: {
							...
							callback: function () {
								...
								api
									.del("/categories/" + ajaxify.data.cid)
									.then(() => {
										// if (intervalId) {
										// 	clearInterval(intervalId);
										// }
					...
					api
						...
						.then(() => {
							var parent = allCategories.filter(function (category) {
								return (
									category &&
									parseInt(category.cid, 10) === parseInt(parentCid, 10)
								);
							});
							parent = parent[0];
