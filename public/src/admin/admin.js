'use strict';
//! this are the functions that are executed in place https://stackoverflow.com/questions/2937227/what-does-function-jquery-mean
(function () {
	var logoutTimer = 0;
	...
	function selectMenuItem(url) {
		require(['translator'], function (translator) { // ! does require function by default looks inside public/src/modules/ ?
			url = url
				.replace(/\/\d+$/, '')
			...
			if (/admin\/general\/dashboard$/.test(url)) {
				...
				if (matches) {
					...
					pageTitle = '[[admin/menu:section-' +
						...
						']]' + (matches[2] ? (' > ' + mainTitle) : '');
					console.log(mainTitle, pageTitle, matches, '----');
